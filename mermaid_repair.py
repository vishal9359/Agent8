...
class MermaidRepair:
    ...
    def _split_compound_statements(self, code: str) -> str:
        """
        Ensure that each Mermaid statement (node declaration or edge)
        starts on its own line.
        
        This is critical because the LLM sometimes generates content like:
            S1([Start])D1{cond} D1 -->|Yes| P1
        which Mermaid parses as invalid. We generically insert newlines
        between closing brackets/braces and the next statement.
        
        This logic is fully generic and does not rely on any project-
        specific names; it only looks at Mermaid syntax patterns.
        """
        # Insert a newline between a closed node and the next node/edge
        patterns = [
            # Node close (]) or end of special node ([...]) or brace (})
            # followed by another node declaration: ID[...], ID{...}, ID([...])
            (r'(\]\)|\]|\})\s*([A-Za-z0-9_]+\(\[)', r'\1\n\2'),
            (r'(\]\)|\]|\})\s*([A-Za-z0-9_]+\[)', r'\1\n\2'),
            (r'(\]\)|\]|\})\s*([A-Za-z0-9_]+\{)', r'\1\n\2'),
            # Node close followed by an edge definition starting with an ID
